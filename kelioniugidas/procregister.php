  ...
  $_SESSION['prev'] = "procregister";
		...
		{ // vardas  geras,  nuskaityti vartotoja is DB
		 ...
		 $pass=substr(hash('sha256',$pass),5,32);     // DB password skirti 32 baitai, paimam is maisos vidurio 
		            ...
		            // nustatom default role
            $ulevel = $user_roles[DEFAULT_LEVEL];

            // jei žymima varnelė "gidas" ir registracija neadmin, pakeičiam į 5
            if (!empty($_POST['is_guide']) && $_POST['is_guide'] == 1 && $_SESSION['ulevel'] != $user_roles[ADMIN_LEVEL]) {
                $ulevel = 5;
            }

            // jei registruoja adminas, paimti jo nurodytą rolę
            if ($_SESSION['ulevel'] == $user_roles[ADMIN_LEVEL] && !empty($_POST['role'])) {
                $ulevel = $_POST['role'];
            }
